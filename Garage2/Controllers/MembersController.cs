...
namespace Garage2.Controllers
{
    public class MembersController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Search([Bind(Include = "FirstName, LastName")] Member m1)
        {
            string fn = m1.FirstName;
            string ln = m1.LastName;
            var model =
               from p in db.Members
               where (p.FirstName == fn || p.LastName == ln)
               select p;
            return View("FindMember", model);    // Back to FindMember view !
        }
